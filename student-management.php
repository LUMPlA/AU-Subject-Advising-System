...
<body style="background-color: #dfdfdf;">
      <section style="background: #dfdfdf; position: relative; left: 0; width: 100%; background-color: #dfdfdf; color: #333; padding: 0px 0px; box-shadow: 0 4px 5px rgba(0, 0, 0, 0.1); z-index: 1000;" class="header">
            <div class="container-fluid">
                ...
                <nav class="navbar navbar-expand-md navbar-dark">
                    <a style="color: #294D86; font-weight:bold" class="navbar-brand heading-black">AUSAS - Students Management</a>
                    <!-- Toggle Button -->
    ...
    height: 30px;
    align-items: center;
    color: white;'>Student account created!</p></div>
";
  ...
  echo "<div id='notification' style='visibility: visible;'><p class='notif' style='color: white; text-align: center; font-weight: 600; position: fixed;
    ...
    height: 30px;
    align-items: center;
    color: white;'>Student account deleted!</p></div>
";
        }
 ...
 echo "<div id='notification' style='visibility: visible;'><p class='notif' style='color: white; text-align: center; font-weight: 600; position: fixed;
    ...
    height: 30px;
    align-items: center;
    color: white;'>Student account updated!</p></div>
";
        }
    ...
    <form id="login-form" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="POST">
        ...
        <input type = "submit" name = "btnsearch" value = "Search">
            ...
            <li style="text-align: right; margin-right: 45px; color: #333;"><a style="color: #333;" class="create-new-account" href="create-student.php"><i style="margin-right: 10px; color: #333;" class="fas fa-user-plus"></i>Create New Student Account</a></li>
...
function buildtable($result){
    if(mysqli_num_rows($result) > 0){
        ...
        while ($row = mysqli_fetch_array($result)){
            echo "<tr data-username='" . $row['studentNumber'] . "'>";
            ...
            echo "<a class='update' href='update-student.php?username={$row['studentNumber']}'>Update</a>";
            echo "<a style='color: #fff;' class='delete' onclick='confirmDelete(\"{$row['studentNumber']}\")'>Delete</a>";
            echo "</td>";
            echo "</tr>";
        }
        echo "</table>";
...
if(isset($_POST['btnsearch'])){
    $sql = "SELECT * FROM stdaccounts WHERE studentNumber LIKE ? OR lastName LIKE ? ORDER BY lastName";
    ...
    $sql = "SELECT * FROM stdaccounts ORDER BY lastName";
    if ($stmt = mysqli_prepare($link, $sql)) {
        if (mysqli_stmt_execute($stmt)) {
            $result = mysqli_stmt_get_result($stmt);
            buildtable($result);
...
<footer >
 ...
 <div id="deleteModal" class="modal">
        <div class="modal-content">
            ...
            <p>Are you sure you want to delete the account with username <span id="deleteUsername"></span>?</p>
                    ...
                    if (xhr.readyState == 4 && xhr.status == 200) {
                        ...
                        if (result === "success") {
                            location.reload(); // Refresh the page after successful deletion
                             ...
                             window.location.href = 'student-management.php?delete_success=1';  
